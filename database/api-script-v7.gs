  ...
  try {
    ...
    switch (payload.action) {
      ...
      case 'ingredients.delete':
        assertRemovable_('ingredients', payload.data.id || payload.id, 'Ingrediënt');
      ...
      case 'recipes.delete':
        ...
        return jsonResponse({ ok: true });
      case 'processed_products.save':
        return jsonResponse({ ok: true, data: saveProcessedProduct_(payload.data) });
      case 'processed_products.delete':
        deleteProcessedProduct_(payload.data.id || payload.id);
        return jsonResponse({ ok: true });
      case 'boxes.save':
      ...
      case 'images.delete':
        assertRemovable_('images', payload.data.id || payload.id, 'Afbeelding');
      ...
      case 'stock.adjust':
        return jsonResponse({ ok: true, data: applyStockAdjustments_(payload.data || {}) });
...
function saveProcessedProduct_(item) {
  if (!item || !item.id || !item.name) throw new Error('Processed product requires id and name');
  const row = {
    id: item.id,
    name: item.name,
    process_type: item.processType || item.process_type || '',
    source_item_1: item.sourceItem1 || item.source_item_1 || '',
    source_amount_1: Number(item.sourceAmount1 || item.source_amount_1 || 0),
    source_item_2: item.sourceItem2 || item.source_item_2 || '',
    source_amount_2: Number(item.sourceAmount2 || item.source_amount_2 || 0),
    yield: Number(item.yield || 0),
    unit: item.unit || '',
    active: item.active !== false,
    notes: item.notes || ''
  };
  saveRowById_('processed_products', row);
  return mapProcessedProductRow_(row);
}
...
function deleteRecipeExpanded_(id) {
  assertRemovable_('recipes', id, 'Recept');
...
function getIngredientMap_() {
  var map = {};
  listRows_('ingredients').forEach(function(row){ map[row.id] = row; });
  return map;
}
function appendStockLog_(entry) {
  var sh = getSheet_('stock_logs');
  sh.appendRow([
    entry.timestamp || new Date().toISOString(),
    entry.action || '',
    entry.item_id || '',
    Number(entry.delta || 0),
    Number(entry.old_value || 0),
    Number(entry.new_value || 0),
    entry.note || '',
    entry.user || 'app'
  ]);
}
function adjustIngredientStock_(itemId, delta, action, note, user) {
  var ingredients = getIngredientMap_();
  var row = ingredients[itemId];
  if (!row) throw new Error('Ingrediënt niet gevonden: ' + itemId);
  var oldValue = Number(row.stock || 0);
  var newValue = oldValue + Number(delta || 0);
  if (newValue < 0) throw new Error('Stock kan niet negatief worden voor ' + (row.name || itemId));
  saveRowById_('ingredients', {
    id: row.id,
    name: row.name || '',
    type: row.type || '',
    category: row.category || '',
    ...
    active: normalizeBool_(row.active),
    image: row.image || '',
    calories_per_processed_piece: Number(row.calories_per_processed_piece || 0),
    processed_yield: Number(row.processed_yield || 0),
    price_per_processed_piece: Number(row.price_per_processed_piece || 0),
    weight_per_piece_g: Number(row.weight_per_piece_g || 0),
    price_per_calorie: Number(row.price_per_calorie || 0),
    notes: row.notes || ''
  });
  appendStockLog_({
    action: action,
  ...
  return { itemId: itemId, oldValue: oldValue, newValue: newValue, delta: Number(delta || 0) };
}
function applyStockAdjustments_(payload) {
  var action = payload.action || 'manual_adjust';
  var note = payload.note || '';
  var adjustments = payload.adjustments || [];
  return adjustments.map(function(adj){
    return adjustIngredientStock_(adj.itemId || adj.item_id, Number(adj.delta || 0), action, adj.note || note, 'app');
  });
}
function listStockLogs_() {
  return listRows_('stock_logs').map(function(row){
    return {
...
function collectDependencies_() {
  var deps = { ingredients: {}, processed_products: {}, recipes: {}, images: {} };
  function push(bucket, id, label) {
    if (!id) return;
    if (!deps[bucket][id]) deps[bucket][id] = [];
    deps[bucket][id].push(label);
  }
  listRows_('processed_products').forEach(function(row){
    push('ingredients', row.source_item_1, 'processed_products:' + row.name);
    push('ingredients', row.source_item_2, 'processed_products:' + row.name);
    push('processed_products', row.source_item_1, 'processed_products:' + row.name);
    push('processed_products', row.source_item_2, 'processed_products:' + row.name);
  });
  listRecipesExpanded_().forEach(function(recipe){
    (recipe.ingredients || []).forEach(function(line){
      push('ingredients', line.id, 'recipes:' + recipe.name);
      push('processed_products', line.id, 'recipes:' + recipe.name);
    });
    push('images', recipe.image, 'recipes:' + recipe.name);
  });
  listRows_('ingredients').forEach(function(item){ push('images', item.image, 'ingredients:' + item.name); });
  listBoxesExpanded_().forEach(function(box){
    (box.items || []).forEach(function(id){ push('recipes', id, 'boxes:' + box.name); });
    push('images', box.image, 'boxes:' + box.name);
  });
  listRows_('menu_settings').forEach(function(row){
    if (row.key === 'menu_logo_image' || row.key === 'menu_background_image') push('images', row.value, 'menu_settings:' + row.key);
  });
  return deps;
}
function assertRemovable_(bucket, id, label) {
  var refs = (collectDependencies_()[bucket] || {})[id] || [];
  if (refs.length) throw new Error(label + ' is nog in gebruik: ' + refs.join(', '));
}
function deleteProcessedProduct_(id) {
  assertRemovable_('processed_products', id, 'Verwerkt product');
  deleteRowById_('processed_products', id);
}
